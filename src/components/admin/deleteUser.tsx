...
export const DeleteUser = ({ isModule, setModule, reload, id }: type) => {
    ...
    const handleSubmit = () => {
        setLoading(true)
        setTimeout(() => {
            axios.post(`${API_URL}/admin/delete/user`, { token, id }).then((result) => {
        ...
        <Module isModule={isModule} setModule={setModule}>
            <div className='p-4 flex flex-col gap-4 justify-center'>
                ...
                <div className='flex items-center justify-center px-4'>
                    <p className='text-center text-xl tracking-wide'>Are you sure you want to delete <span className='font-bold capitalize'>{user[0]?.first_name} {user[0]?.last_name}</span>?</p>
                </div>
